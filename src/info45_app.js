...
class IndecisionApp extends React.Component {
    ...
    // handleDeleteOptions() {     // longer version of method above
    //     this.setState(() => { 
    //         return {
    //             options: []
    //         }
    ...
    handleAddOption(option) {
        ...
        //     return {    // use array concat to give new array by merging array values;  cant use push as that directly updates array
        //         options: prevState.options.concat(option)   // same as .concat([option])
        //     }
        // })
    }
...
/* set default for props.option for <IndecisionApp /> -- removed once we moved to using LocalStorage */
...
// class Options extends React.Component {  // render new <p> tag for each option w/in this.props.options
//     render() {
//         return (
//             <div>
//             {/* .bind(this) refers to the same 'this' as render() & will allow inner this.props references to work */}
//                 {/* <button onClick={this.handleRemoveAll.bind(this)}>Remove All</button>  [inefficient] */}
...
class AddOption extends React.Component {
    ...
    handleAddOption(e) {
        e.preventDefault(); 
//        const option = document.querySelector("input[name='option']").value;  // gets data from Input Text field
...
/*const User = (props) => {
    return (
        <div>
            <p>Name: {props.name}</p>  // stateless functional components use props.name instead of this.props.name 
            <p>Age: {props.age} </p>
        </div>
    );   
}
ReactDOM.render(<User name="Andrew" age={26}/>, document.getElementById('app'))  // Renders stateless Functional component 
*/
